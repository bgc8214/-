...
public class ExpandAdapter extends BaseExpandableListAdapter {
    ...
    private ImageView mCountLimit;
    private ImageView mDivideProduct;
    private ImageView mTimeLimit;
    private ImageView mNewPanel;
    private ImageView mDeletePanel;
    ...
    public View getGroupView(final int groupPosition, final boolean isExpanded, View convertView, final ViewGroup parent) {
        ...
        mCountLimit = (ImageView) convertView.findViewById(R.id.countLimitPanel);
        mTimeLimit = (ImageView) convertView.findViewById(R.id.timeLimitPanel);
        mDivideProduct = (ImageView) convertView.findViewById(R.id.divideProductPanel);

        mNewPanel = (ImageView) convertView.findViewById(R.id.newPanel);
        mDeletePanel = (ImageView) convertView.findViewById(R.id.deletePanel);


        String etc = DataList.get(groupPosition).getEtc();
        String[] list = etc.split(",");

        //Log.d("ExpandAdapter", etc);
        for (int i = 0; i < list.length; i++) {
            switch (list[i].trim()) {
                case "횟수제한":
                    mCountLimit.setVisibility(View.VISIBLE);
                    break;
                case "기간한정":
                    mTimeLimit.setVisibility(View.VISIBLE);
                    break;
                case "분할제공":
                    mDivideProduct.setVisibility(View.VISIBLE);
                    break;
                default:
                    mDivideProduct.setVisibility(View.GONE);
                    mTimeLimit.setVisibility(View.GONE);
                    mCountLimit.setVisibility(View.GONE);
                    break;
            }
        }

        //TODO 실제 데이터가 들어가면 NULL 처리 해제

        String state = DataList.get(groupPosition).getState();
        if(state == null) state = "able";
        switch (state.trim()) {
            case "new":
                mNewPanel.setVisibility(View.VISIBLE);
                mDeletePanel.setVisibility(View.GONE);
                break;
            case "end":
                mDeletePanel.setVisibility(View.VISIBLE);
                mNewPanel.setVisibility(View.GONE);
                break;
            case "able":
                mNewPanel.setVisibility(View.GONE);
                mDeletePanel.setVisibility(View.GONE);
